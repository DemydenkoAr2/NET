...
namespace SecondLesson.oop
{
	...
	class Person
	{
		...
		// init — это модификатор доступа, который появился в C# 9.0.
		// Он означает, что свойство TestInfo можно инициализировать
		// только во время создания объекта, но нельзя изменить его значение после этого.
		public string TestInfo { get; init; } = "Some test info";

		// Пример использования:
		// var obj = new MyClass { TestInfo = "New value" }; // Можно задать значение при создании
		// obj.TestInfo = "Another value"; // Ошибка! Нельзя изменить после инициализации
	}
	...
	public class PersonV2
	{
		...
		public PersonV2(string name, string hobby, string hobbyPrivate)
		{
			Name = name;
			Hobby = hobby;
	...
	public class Employee : PersonV2 // наследуемся от класса Person
	{
	...
	public static class OOPExamples
	{
		...
		{
			var phone = new SmartPhone("Phone 123", 1234.123m, 12.5);
		}
	}
}
